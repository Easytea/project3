    ...
    if($_GET["action"] == "delete")
    {
        foreach($_SESSION["shopping_cart"] as $keys => $values)
        {
            if($values["item_id"] == $_GET["id"])
            {
                ...
                echo '<script>window.location="indexshirts.php"</script>';
            }
        }
    }
}
...
<div class="container">
    <?php
    $query = "SELECT * FROM tbl_product WHERE object = 'shirt' ORDER BY id ASC";
    ...
    if(mysqli_num_rows($result) > 0)
    {
        while($row = mysqli_fetch_array($result))
        {
            ?>
            <div class="col-md-4">
                <form method="post" action="indexshirts.php?action=add&id=<?php echo $row["id"]; ?>">
                    <div class="product">
            ...
            if(!empty($_SESSION["shopping_cart"]))
            {
                ...
                foreach($_SESSION["shopping_cart"] as $keys => $values)
                {
                    ...
                    <tr>
                        ...
                        <td><a href="indexshirts.php?action=delete&id=<?php echo $values["item_id"]; ?>"><span class="text-danger">Verwijder</span></a></td>
                    </tr>
                    <?php
